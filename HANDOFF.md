...
Le projet est en **v6.18 (2 onglets — Marchés + Veille sport business)** depuis le 18/05/2026 : déblocage de la saturation du top-N (cap codé en dur 200, atteint depuis le 8 mai → empêchait les nouveaux items d'entrer si le top était plein). Cap relevé à 1000, purge 90j rendue inconditionnelle (la branche `not source_auto or ...` était inerte en pratique), log `[PURGE 90j]` ajouté pour tracer la rotation. Version précédente v6.17 (27/04/2026) : 3 cosmétiques P3 — favicon fallback (pastille dorée si Google S2 retourne du vide), libellé de catégorie dans le modal détail, extension de `WHITELIST_EMETTEURS_LOCAUX_FR` avec 14 métropoles régionales + 19 départements importants (194 entrées au total).
...
**Priorité haute** :
- **P0 — 14 sources RSS marchés publics à 0 entrée** (identifié 18/05/2026 en session v6.18). Sur le dernier run prod, 14 feeds BOAMP/TED/FM remontent systématiquement 0 entrée alors que les URLs sont bien live en curl. Forte présomption de 403 silencieux côté feedparser (les datacenters GitHub Actions sont souvent blacklistés sans UA navigateur). Fix proposé : remplacer `feedparser.parse(url)` par un fetch `requests.get(url, headers={"User-Agent": "Mozilla/5.0..."})` puis `feedparser.parse(response.content)`. Tester avec un seul feed BOAMP avant déploiement.
- **P1 — Aucune requête SerpAPI ne cible les domaines fédés** (ffbad.org, ffr.fr, fft.fr...). Angle mort sur les AO publiés directement sur les profils acheteurs fédéraux. Ajouter une requête consolidée `site:ffbad.org OR site:fft.fr OR site:ffr.fr OR site:ffbb.com OR site:fff.fr ("appel d offre" OR "consultation" OR "marche public")` dans `GOOGLE_QUERIES` (coût : +1 req × 22j = +22/mois sur quota SerpAPI 100, restera dans le budget).
- **P2 — Fédérations qui scrapent mais filtrent à zéro** : FFF/FFBB/FFN/FFR13/FFVolley/FFE remontent des blocs HTML mais 0 item passe les filtres downstream → investiguer dans `categorize()` + `pre_filter()` ce qui drop tout. FFR/FFT/FFJudo/FFHandisport/FFVoile/FFBad/FFME à `0 blocs` au fetch → soit le sélecteur CSS est obsolète, soit la page de consultations a déménagé.
- ~~Vérifier que les 4 nouvelles URLs RSS cat.3 fonctionnent~~ **FAIT 26/04/2026** : 4 sources mortes identifiées et retirées en v6.13 (newstanksport.fr DNS dead, sportexpertise.com DNS dead, lecafedusportbiz.fr 404 site Beehiiv sans RSS, lequipe.fr/rss/actu_rss.xml 404 RSS publics décommissionnés). Les autres sources cat.3 (SBC ×3, SPORSORA ×2, Sport Buzz Business, Sport Stratégies, COSMOS, News Tank fallback HTML) répondent toutes 10-20 entrées.
- ~~Confirmer que `data/meta.json` est bien généré avec un `system_version` non-vide (format `v6 · abc1234`)~~ **FAIT 26/04/2026** : OK, `"v6 · 55d75a1"` dans le run du 26/04 07:51 UTC. Format `v6 · sha` validé comme convention figée (pas v6.12·sha — décision Cyril 26/04).
- ~~Confirmer que le bloc meta s'affiche correctement dans le header de la page d'accueil après refresh~~ **FAIT 26/04/2026** : validation côté HTML servi par GitHub Pages : bloc `<div class="meta-block">` présent avec spans `#metaUpdated` et `#metaVersion`, JS qui fetch `data/meta.json?t=…` et injecte les valeurs. `meta.json` live retourne `system_version: "v6 · <sha>"` + `updated_at_human` corrects. Rendu attendu en nav : "Données M.A.J 26/04/26 à HHhMM · Version v6 · <sha>". Si jamais le bloc reste vide visuellement après un run, c'est forcément un cache navigateur (Cmd+Shift+R, cf. piège #3).
...
**Sécurité** :
- ~~Token PAT GitHub `ghp_AVGhVg…` exposé dans l'URL du remote~~ **CLOS 26/04/2026** : (1) URL du remote nettoyée (`https://github.com/cyrilmourin/sideline-veille.git`), (2) auth git désormais via `gh` credential helper + keychain macOS (token OAuth `gho_…` scopes `repo, workflow, gist, read:org`), (3) PAT classic `ghp_AVGhVg…` révoqué manuellement par Cyril sur github.com/settings/tokens. Auth post-révocation testée OK (fetch + `gh api user`).
...
**11. Saturation silencieuse du cap top-N (v6.18).**
Avant v6.18, `sorted(...)[:200]` capait le JSON final à 200 items sans warning. Conséquence : dès que la prod atteignait 200 (cas observé du 8 au 18/05/2026, 10 jours), un nouvel item ne pouvait entrer que s'il battait au score un item déjà présent — donc les AO récents avec score moyen restaient invisibles malgré leur passage en `seen_ids`. Le piège c'est qu'il n'y avait aucune trace de l'éviction dans le log : `count_total: 200` jour après jour ressemblait à un palier stable, pas à une saturation. v6.18 monte le cap à 1000 et la purge 90j (rendue inconditionnelle) fait le vrai garde-fou. Si le cap remonte un jour à saturation, ajouter un log d'alerte `[CAP HIT] N items evinces` dans le `sorted()[:N]`.

---

## Historique

- 2026-05-18 : v6.18 livrée — déblocage saturation top-N. Diagnostic : depuis le 8 mai (10 jours), `meta.json.count_total` plafonne à 200 jour après jour, ce qui empêche tout nouvel item d'entrer s'il ne bat pas au score un item déjà présent. Identifié en cherchant pourquoi l'AO « Boutique officielle FFBAD » du 17/05 ne remontait pas. (a) **Cap 200 → 1000** sur `sorted(...)[:1000]` (scraper.py:2308). Marge ×5, la purge 90j fait le vrai garde-fou. (b) **Purge 90j explicite** sur tous les items : la condition `not o.get("source_auto") or ...` rendait la purge facultative pour les items non-auto, mais en pratique tous les items en base ont `source_auto: true` → branche inerte. Simplifié en `_parse_date(...) > cutoff` sec, plus de double négation. (c) Log `[PURGE 90j] N -> M (X items >3 mois supprimes)` ajouté pour tracer la rotation à chaque run. Bump version : header v6.17→v6.18, banner runtime `Sideline Veille v6.18 -- Demarrage`, argparse description. PR #6 (branche `claude/fix-missing-alert-xz5cM`) mergée. **Sujets restants non couverts ici** (à scinder en PR séparés sur décision) : P0 — 14 sources RSS marchés publics (BOAMP/TED/FM) à `0 entrees` systématiquement, forte présomption de 403 silencieux feedparser (fix proposé : UA navigateur sur les requêtes feedparser) ; P1 — aucune requête SerpAPI ne cible les domaines fédés (ffbad.org, ffr.fr, fft.fr...) → angle mort sur les AO publiés directement sur les profils acheteurs fédéraux ; P2 — FFF/FFBB/FFN/FFR13/FFVolley/FFE scrapent des blocs mais 0 item passe les filtres downstream, et FFR/FFT/FFJudo/FFHandisport/FFVoile/FFBad/FFME à `0 blocs` au fetch.
- 2026-04-27 (suite 3) : v6.17 livrée — les 3 cosmétiques P3 du HANDOFF. (a) **Favicon fallback** : `index.html` ligne 475, l'`onerror` qui mettait `visibility:hidden` (laissait un trou de 56px) est remplacé par un swap `outerHTML` vers une div `.opp-favicon-fallback` (pastille dorée 56x56 avec losange `◆` central, charte `--or`/`--or-pale`). Même rendu si l'item n'a pas de `faviconUrl`. (b) **Modal IA contexte catégorie** : ajout d'un `info-item` "Catégorie" dans la grille du modal, qui affiche "Marché public" pour cat.1, "Veille sport business" pour cat.3 (et "Signal" pour cat.2 legacy par sécurité, même si plus émis). (c) **Whitelist départements/métropoles** : `WHITELIST_EMETTEURS_LOCAUX_FR` passe de 120 à 194 entrées. Ajout de 14 métropoles régionales (Montpellier, Rouen, Tours, Grenoble, Saint-Étienne, Clermont-Ferrand, Toulon, Orléans, Dijon, Nancy, Metz, Reims, Brest, Caen) et 19 départements importants : tout l'Île-de-France (75/77/78/91/92/93/94/95), Nord (59), Pas-de-Calais (62), Rhône (69), Bouches-du-Rhône (13), Haute-Garonne (31), Gironde (33), Var (83), Alpes-Maritimes (06), Isère (38), Hérault (34), Ille-et-Vilaine (35), Loire-Atlantique (44). Chaque département a 4 variantes ("departement de X", "conseil departemental NN", "cd NN", forme courte). Toutes les TODO P3 du HANDOFF sont closes.
- 2026-04-27 (suite 2) : v6.16 livrée — couverture des deux TODO P2 ouvertes la veille. (a) **Bug BOAMP API** : `lancer_boamp_api()` extrayait bien `date_lim = rec.dlrremplaco or rec.datelimitereception` (ligne 1209) mais l'oubliait dans le dict d'item retourné. Conséquence : tous les items BOAMP API avaient `dateLimite=None` depuis l'origine. Fix : ajout de `"date_limite": date_lim or None` dans le dict. Note : sur le marché existant `26_40704`, l'API upstream retourne elle-même `None` pour les deux champs date — donc la migration du stock n'a pas été possible. Le fix vaut pour les futurs items. (b) **Dispatcher générique** : refactor de `enrich_achatpublic_date` derrière un point d'entrée `enrich_date_limite_from_link(lien)` qui route selon le domaine. Couvre `achatpublic.com` aujourd'hui ; squelette préparé pour étendre à `maximilien.fr` et `cnosf.e-marchespublics.com` (commenté dans le code) le jour où un AO concret apparaîtra dans le JSON et permettra de calibrer la regex. Pas étendu maintenant car les listings Maximilien/CNOSF étaient vides. (c) **Filtre dateLimite passée à la lecture** : `charger_donnees()` drop les items `category in (1,2)` dont `dateLimite < today` à chaque démarrage du scraper. Cat.3 conservée intacte (pas de notion de deadline). Effet : un AO clos disparaît automatiquement le lendemain de sa deadline, sans attendre re-traitement. Tests locaux 5/5 OK : dispatcher (achatpublic→date, hors-cible→None), filtre (AO clos drop, AO en cours keep, AO same-day keep, cat.3 bypass, AO sans date keep).
- 2026-04-27 (suite) : v6.15 livrée — enrichissement `dateLimite` pour les liens `achatpublic.com`. Cyril remonte un AO Marches 2030 (Prestations de veille COJOP Alpes 2030) sans `dateLimite` alors que la page achatpublic indique "Date limite de remise des plis : 29 avril 2026 12:00". Cause : le scraper Marches 2030 (HTML) extrait juste titre+desc+lien depuis la page liste, qui n'expose pas la dateLimite ; le détail vit sur la page achatpublic pointée. Fix : ajout d'une fonction `enrich_achatpublic_date(lien)` qui fetche la page achatpublic (timeout 8s, silent on error) et regex la "Date limite de remise des plis" + dictionnaire `_MOIS_FR` + `_parse_fr_date()` pour convertir "29 avril 2026" → "2026-04-29". Hook dans `parse_html()` après création de l'item : si lien `achatpublic.com`, on enrichit. Coût : +1 fetch HTTP par bloc Marches 2030 (~25 max), tolérable. Migration one-shot des items achatpublic existants dans `data/opportunites.json` : 1 item enrichi (COJOP veille, dateLimite=2026-04-29, urgent=True). Tests unitaires locaux : `_parse_fr_date` 4/4 OK, `enrich_achatpublic_date` retourne 2026-04-29 sur l'URL réelle. **À faire en P2** : étendre l'enrichissement à d'autres profils acheteurs (BOAMP HTML, Maximilien, CNOSF e-marchespublics) — pour l'instant seul achatpublic est couvert. Aussi : prévoir un filtre `dateLimite < today` au chargement du JSON pour purger automatiquement les AO clos (actuellement ce filtre ne s'applique qu'au scraping de nouveaux items).
- 2026-04-27 : v6.14 livrée — resserrage filtre titre fédération. Cyril a remonté un faux positif `https://www.ffr13.fr/pacifique-xiii-lance-lappel-du-large/` en cat.1. Cause : le `pre_filter()` étape 10 (filtre titre fédération) acceptait le mot "appel" seul → "appel du large" matchait. Fix : remplacement par les formes prépositionnelles explicites (`appel d offre`, `appel a candidature`, `appel a projet`, `appel a la concurrence`, `appel a manifestation`, `appel a propositions`). Bonus ceinture/bretelle : ajout de "appel du large", "appel a l aide", "lance l appel" dans `ACTU_PURE_PENALTY` pour drop si jamais une autre source contourne le filtre. Tests locaux 4/4 OK (drop Pacifique XIII, pass AO appel à concurrence/candidature/consultation). Item Pacifique XIII (id `27490ad3fdb5`, score 24) purgé manuellement de `data/opportunites.json` ; meta.json passe de 4/0/64 à 3/0/64 cat1/2/3.
- 2026-04-26 (après-midi, 1ère session Claude Code, clôture) : (a) Validation finale TODO P1 « bloc meta dans le header » — fetch direct de `https://cyrilmourin.github.io/sideline-veille/` + `data/meta.json` live, JS et DOM cohérents, `system_version: "v6 · 92a5a62"` correctement servi. (b) Création de `.claude/settings.json` versionné (allowlist 57 entrées + deny 11 entrées) pour réduire les prompts d'autorisation Claude Code sur le workflow normal (git, gh, curl, python3, inspection). (c) Skill `/fewer-permission-prompts` lancée pour audit : aucun pattern read-only sûr non-couvert ne ressortait — l'allowlist initiale était bien dimensionnée. Toutes les TODO Priorité haute du HANDOFF sont closes. Sujet sécurité (PAT) clos. Restent uniquement des TODO P2/P3 non urgentes.
- 2026-04-26 (après-midi, 1ère session Claude Code, suite) : Bascule auth git du PAT classic vers `gh` credential helper. URL du remote nettoyée (token retiré), `gh auth git-credential` (déjà configuré dans `~/.gitconfig` côté Cyril) prend le relais via keychain macOS. PAT `ghp_AVGhVg…` révoqué manuellement par Cyril sur github.com/settings/tokens. Auth post-révocation validée. Sujet sécurité clos.
- 2026-04-26 (après-midi, 1ère session Claude Code) : v6.13 livrée — nettoyage des sources cat.3 mortes. Audit en local (avantage Claude Code vs Cowork sandbox) des URLs RSS cat.3 du dernier run prod (24951571903). 4 sources à 0 entrées identifiées : `newstank_sport` (newstanksport.fr DNS dead), `gie_sport_expertise` (sportexpertise.com DNS dead), `cafe_sport_biz` (lecafedusportbiz.fr → site Beehiiv, pas de RSS public exposé), `lequipe_sport_business` (lequipe.fr/rss/actu_rss.xml 404, l'Équipe a décommissionné les RSS publics). Suppression des 4 dans `SOURCES` + nettoyage des entrées correspondantes dans `SOURCE_CATEGORY`. Le fallback HTML `newstank_sport_home` (sport.newstank.fr/home) est conservé. Bug bonus repéré : `system_version` au format `v6·sha` alors que CAHIER_DES_CHARGES prescrivait `v6.12·sha`. Décision Cyril : on garde `v6 · sha` figé (option 1) → CAHIER_DES_CHARGES corrigé en 3 endroits (sample meta.json §3.2, header §7.1, step CI §8.2). Aucune modif workflow ni fallback Python (déjà à `v6`). Distribution attendue après prochain run : ~64 cat.3 stable (les 4 sources mortes étaient déjà à 0).
- 2026-04-26 (matin) : v6.11 + v6.12 livrées. (a) Refonte scoring métier Sideline : `METIER_SIDELINE_BONUS` (+8 à +22pts par mot-clé AP/conseil/com/sponsoring/droit du sport, cap 3 hits) + `ACTU_PURE_PENALTY` (−25pts par hit résultats/billetterie/finales/transferts). (b) Suppression des sources d'actu sportive pure (NBA, Roland Garros, Tour de France, F1, FFA, LFP, LNR) qui polluaient cat.3. (c) Filtre titre AO obligatoire pour les sources `type=federation` (drop "Accueil", "Finale Coupe", "Billetterie"…). (d) Dédup multi-source par `(titre normalisé + émetteur)` qui complète la dédup par URL canonique (un même AO peut être publié sur BOAMP RSS + BOAMP API + France Marchés avec 3 URLs différentes — Cyril voyait des triplons). (e) Fédérations classées cat.1 (au lieu de cat.3 par fallback). (f) Purge JSON one-shot des 35 items legacy hors-cible. (g) **v6.12** : fusion cat.2 dans cat.3, frontend passe de 3 onglets à 2 (Marchés + Veille). Le champ `category=2` reste toléré côté JS pour rétrocompat sur les items legacy.
