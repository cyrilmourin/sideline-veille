...
Le projet est en **v6.16 (2 onglets — Marchés + Veille sport business)** depuis le 27/04/2026 : fix bug BOAMP API (la `date_limite` était extraite mais jamais incluse dans l'item retourné), ajout d'un dispatcher générique d'enrichissement `dateLimite` (squelette extensible Maximilien/CNOSF), et drop automatique des AO clos au chargement du JSON (`charger_donnees()` filtre `dateLimite < today` pour cat.1/2, bypass cat.3).
